...
/**
 * ICD-ICF-ICHI Section for Audiology Assessment
 * 
 * API:
 * GET /api/codes/icd/{department}/
 * Response: { count, next, previous, results: [{ code, name, icf: [...], ichi: [...] }] }
 * 
 * Shows: mode="icd-icf" -> ICD Selection + ICF Items
 */

function ICDICFICHISection({ values = {}, onChange, mode = "icd-icf" }) {
  const DEPARTMENT = "Audiology";
  const showICF = mode === "icd-icf";
  ...
  const [selectedICDs, setSelectedICDs] = useState(() => {
    // Try to get from current values first
    if (values.selected_icds) return values.selected_icds;
    // For Plan tab, it might be stored here
    return [];
  });
  ...
  return (
    <div style={S.section}>
      {/* ICD Selection - only show in Assessment tab */}
      {showICF && (
        <>
      ...
      {/* ICF Items - only show in Assessment tab */}
      ...
      {/* ICHI Items - only show in Plan tab */}
      {showICHI && allICHIItems.length > 0 && (
        <div style={S.subsection}>
          <div style={S.subheading}>ICHI Items</div>
...
export default ICDICFICHISection;
